...
describe("worker/patches: push aggregation and protected branch fallback", () => {
  it("pushes to multiple remotes; falls back to topic branch for grasp-like remote and aggregates pushErrors", async () => {
    ...
    const remoteOk = new VirtualGitRemote({
      fs: fsRemoteOk as any,
      dir: "/remote-ok",
      defaultBranch: "main",
    ...
    const remoteProt = new VirtualGitRemote({
      fs: fsRemoteProt as any,
      dir: "/remote-prot",
      defaultBranch: "main",
      author: {name: "R2", email: "r2@example.com"},
    })
  ...
  it("handles protected-branch fallback outcomes for grasp remotes", async () => {
    const registry = createRemoteRegistry()
    ...
    const hasFallbackPush = (res.pushedRemotes || []).some(r => r.startsWith("prot:grasp/patch-"))
    const fallbackError = (res.pushErrors || []).find(e => e.remote === "prot")

    expect(hasFallbackPush || Boolean(fallbackError)).toBe(true)

    if (fallbackError) {
      expect((res.skippedRemotes || []).includes("prot")).toBe(true)
      expect(fallbackError.error.toLowerCase()).toContain("fallback")
    }
