...
describe("NIP-34 parsers (nip34-utils)", () => {
  ...
  it.todo(
    "should accept ISO created_at strings (implementation gap: parsers currently assume integer seconds)",
  )
  ...
  it("parsePullRequestUpdateEvent parses tip commit, merge base, and NIP-22 E/P", () => {
    ...
    expect(parsed.tipCommitOid).toEqual("c1")
    expect(parsed.mergeBase).toBe("mb")
  })
  ...
  it("parseStatusEvent maps kind to status and extracts related ids", () => {
    ...
    expect(parsed.appliedCommits).toEqual([])
    expect(parsed.mergedCommit).toBeUndefined()
  })
