...
describe("GRASP Full Cycle Integration Tests", () => {
  ...
  describe("Error Cases", () => {
    ...
    it("handles invalid npub format in getUser", async () => {
      ...
      await expect(provider.getUser("invalid-username")).rejects.toThrow(/Invalid user identifier/)
    })
    ...
    it("handles relay without GRASP-01 support", async () => {
      ...
      setPriv(provider, "capabilities", {
        grasp01: false,
        grasp05: true,
        httpOrigins: ["https://relay.test.com"],
      })
      setPriv(provider, "httpBase", "https://relay.test.com")
    ...
    it("handles invalid JSON in profile content", async () => {
      ...
      vi.spyOn(provider as any, "queryEvents").mockResolvedValueOnce([{content: "invalid-json{"}])
    ...
    it("rejects listIssues as unsupported without EventIO", async () => {
      ...
      await expect(provider.listIssues(testPubkeyHex, "test-repo")).rejects.toThrow(/not supported/)
    })

    it("rejects listPatches as unsupported without EventIO", async () => {
      const provider = new GraspApiProvider(testRelayUrl, testPubkeyHex)

      await expect(provider.listPatches(testPubkeyHex, "test-repo")).rejects.toThrow(
        /not supported/,
      )
    })
    ...
    it("rejects forkRepo without EventIO", async () => {
      ...
      await expect(provider.forkRepo(testPubkeyHex, "test-repo")).rejects.toThrow(/not supported/)
    })
  })
...
describe("GraspApi Extended Tests", () => {
  ...
  it("rejects an empty relay list before capability checks", async () => {
    ...
    await expect(api.getCapableRelays([])).rejects.toThrow(
      "GRASP repository operation requires at least one explicit relay",
    )
  })
