...
export async function tryPushWithTokens<T>(
  url: string,
  pushOperation: (authCallback: (() => { username: string; password: string }) | undefined) => Promise<T>
): Promise<T> {
  // Extract hostname
  let hostname: string;
  try {
  ...
  for (const tokenEntry of matchingTokens) {
    try {
      const authCallback = () => ({ username: 'token', password: tokenEntry.token });
  ...
  const error = new Error(`All tokens failed for push operation on ${hostname}. Errors: ${errorMessages}`);
