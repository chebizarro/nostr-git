...
export async function analyzePatchMergeUtil(
  ...
  try {
    ...
    const effectiveTargetBranch = await resolveBranchName(dir, targetBranch || patchData.baseBranch)
...
function applyUnifiedDiffPatch(existingContent: string, patchContent: string): string {
  ...
  for (const h of hunks) {
    ...
    if (aligned === null) {
      throw new Error(
        `Patch context not found at line ${h.oldStart + 1}. Cannot apply patch safely.`,
      )
    }
    advanceTo(aligned)
    ...
    for (const l of h.lines) {
      ...
      if (l.startsWith(" ")) {
        ...
        if (actual !== expected && actual !== undefined) {
          console.warn(
            `Context mismatch at line ${cursor}: expected "${expected}", got "${actual}"`,
          )
        }
  ...
  try {
    ...
    for (const remote of remotes) {
      try {
        ...
        const graspLike = isGraspRepoHttpUrl(remote.url || "")
