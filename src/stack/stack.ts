import type { PatchEvent } from "../events/index.js";
import { getTag, getTagValue } from "../events/index.js";

export type StackMemberRef = {
  id?: string;
  commit?: string;
};
...
export function parseStackEvent(evt: any): StackDescriptor | undefined {
  ...
  const members = (evt.tags as string[][]).filter((t) => t[0] === "member").map((t) => ({ id: t[1] }));
