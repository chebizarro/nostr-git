...
export class RepoCore {
  ...
  static resolveStatusFor(
    ...
    const events = allStatus
      ...
      .filter(
        (ev) => isTrusted(ctx, ev.pubkey) || (!!rootAuthor && ev.pubkey === rootAuthor)
      )
  ...
  static buildRepoSubscriptions(args: { addressA?: string; rootEventId?: string; euc?: string }): RepoSubscriptions {
