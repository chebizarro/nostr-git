import type { VendorProvider, GitVendor, CreateRepoOptions, UpdateRepoOptions } from './vendor-providers.js';
...
class RestVendorProvider implements VendorProvider {
  ...
  getRepoMetadata(owner: string, repo: string, token?: string): Promise<RepoMetadata> {
    if (!token) {
      throw createAuthRequiredError(this.buildContext({ remote: this.originalUrl, operation: 'getRepoMetadata' }));
    }
