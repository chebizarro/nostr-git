    ...
    <div class="p-4">
      ...
      <div class="relative mb-4">
        ...
        <Input
          ...
          class="pl-8 h-8 bg-secondary/50"
            ...
            <Collapsible bind:open={page.isOpen} class="mb-1">
              <div class="flex items-center">
                <button
                  ...
                  on:click={() => toggleSection(page.id)}
                >
                  {#if page.isOpen}
                    ...
                    <div
                      class="flex items-center gap-2 p-2 ml-2 rounded-md text-sm cursor-pointer transition-colors
                             {selectedPage === child.title
                               ...
                               : 'hover:bg-accent/10'}"
                      on:click={() => onSelectPage(child.title)}
                    >
                      <Book class="h-4 w-4" />
            ...
            <div
              class="flex items-center gap-2 p-2 ml-4 rounded-md text-sm cursor-pointer transition-colors
                     {selectedPage === page.title
                       ? 'bg-accent/20 text-accent'
                       : 'hover:bg-accent/10'}"
              on:click={() => onSelectPage(page.title)}
            >
      ...
      <div class="mt-8 space-y-2 pt-4 border-t border-border">
        <button class="flex items-center gap-2 w-full p-2 hover:bg-accent/10 rounded-md text-sm text-muted-foreground">
