<script lang="ts">
  ...
  import type { Patch } from "@nostr-git/core";
        ...
        <div class="rounded-lg border border-blue-200 bg-blue-50 p-4 dark:bg-blue-950/20">
          <div class="flex items-start gap-3">
            ...
            <div>
              ...
              <p class="text-sm text-blue-700 dark:text-blue-300">This patch doesn't modify any files.</p>
            </div>
          </div>
        </div>
      {:else}
        {#if fileStats}
          ...
          <div class="mb-4 rounded-lg border border-muted bg-muted/30 p-4">
            <div class="flex items-center justify-between">
              ...
              <span class="text-xs text-muted-foreground">
                {fileStats.added + fileStats.removed} total line{fileStats.added + fileStats.removed !== 1 ? 's' : ''} affected
              </span>
            </div>
            {#if fileStats.added === 0 && fileStats.removed === 0}
            ...
            {:else}
              <p class="mt-2 text-xs text-muted-foreground">
                Change ratio: {Math.round((fileStats.added / (fileStats.added + fileStats.removed)) * 100)}% additions,
                {Math.round((fileStats.removed / (fileStats.added + fileStats.removed)) * 100)}% deletions
              </p>
            {/if}
          </div>
          ...
          <div class="mb-4 grid grid-cols-1 gap-4 md:grid-cols-3">
            ...
            <div class="rounded-lg border bg-green-50 p-3 text-center dark:bg-green-950/20">
              ...
              <div class="text-xs text-muted-foreground mt-1">
                {fileStats.added === 0 ? 'No additions' :
                 fileStats.added < 10 ? 'Few additions' :
                 fileStats.added < 50 ? 'Moderate additions' : 'Many additions'}
              </div>
            </div>
            ...
            <div class="rounded-lg border bg-red-50 p-3 text-center dark:bg-red-950/20">
              ...
              <div class="text-xs text-muted-foreground mt-1">
                {fileStats.removed === 0 ? 'No deletions' :
                 fileStats.removed < 10 ? 'Few deletions' :
                 fileStats.removed < 50 ? 'Moderate deletions' : 'Many deletions'}
              </div>
            </div>
          </div>
          ...
          <div class="rounded-lg border border-muted bg-muted/30 p-4">
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-2 text-xs">
              {#each selectedPatch.diff as file}
                {@const extension = file.to?.split('.').pop() || file.from?.split('.').pop() || 'unknown'}
                ...
                <div class="flex items-center gap-2 text-sm text-muted-foreground">
                  <span>
                    {selectedPatch.author.name ||
                      selectedPatch.author.pubkey.slice(0, 8)}
                  </span>
                  <span>•</span>
