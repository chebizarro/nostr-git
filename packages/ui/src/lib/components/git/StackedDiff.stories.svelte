              ...
              diffHunks: [
                {
                  ...
                  patches: [
                    {
                      line: "import { Request, Response, NextFunction } from 'express';",
                      type: "+" as const,
                    },
                    { line: "import jwt from 'jsonwebtoken';", type: "+" as const },
                    { line: "", type: "+" as const },
                    {
                      line: "export const authenticateToken = (req: Request, res: Response, next: NextFunction) => {",
                      type: "+" as const,
                    },
                    {
                      line: "  const authHeader = req.headers['authorization'];",
                      type: "+" as const,
                    },
                    {
                      line: "  const token = authHeader && authHeader.split(' ')[1];",
                      type: "+" as const,
                    },
                    { line: "", type: "+" as const },
                    ...
                    { line: "", type: "+" as const },
                    {
                      line: "  jwt.verify(token, process.env.ACCESS_TOKEN_SECRET!, (err: any, user: any) => {",
                      type: "+" as const,
                    },
              ...
              diffHunks: [
                {
                  ...
                  patches: [
                    ...
                    { line: "", type: "+" as const },
                    {
                      line: "  if (!user || !await bcrypt.compare(password, user.password)) {",
                      type: "+" as const,
                    },
                    {
                      line: "    return res.status(401).json({ error: 'Invalid credentials' });",
                      type: "+" as const,
                    },
                    { line: "  }", type: "+" as const },
                    { line: "", type: "+" as const },
                    ...
                    { line: "", type: "+" as const },
                    {
                      line: "  res.json({ accessToken, user: { id: user._id, email: user.email } });",
                      type: "+" as const,
                    },
                    { line: "});", type: "+" as const },
                    { line: "", type: "+" as const },
              ...
              diffHunks: [
                {
                  ...
                  patches: [
                    ...
                    {
                      line: "    for (const [sessionId, session] of this.sessions.entries()) {",
                      type: " " as const,
                    },
              ...
              diffHunks: [
                {
                  ...
                  patches: [
                    ...
                    {
                      line: "import v1UserRoutes from './routes/api/v1/users';",
                      type: "-" as const,
                    },
                    {
                      line: "import v2UserRoutes from './routes/api/v2/users';",
                      type: " " as const,
                    },
                    { line: "", type: " " as const },
                  ],
...
<div class="p-6 space-y-6">
  <div class="border-b border-border pb-4">
    ...
    <p class="text-muted-foreground mb-4">
      A commit-oriented diff viewer inspired by Phabricator and Gerrit, showing each commit as its
      own diff with metadata and file grouping.
    </p>
    ...
    <div class="flex flex-wrap gap-4 text-sm">
      <label class="flex items-center gap-2">
        <input type="checkbox" bind:checked={autoExpandFirst} class="rounded border-border" />
        Auto-expand first commit
      </label>

      <label class="flex items-center gap-2">
        <input type="checkbox" bind:checked={enableVirtualization} class="rounded border-border" />
    ...
    {#if selectedStory === "highlighted"}
      <div class="mt-4">
        <label for="highlighted-files" class="block text-sm font-medium mb-2"
          >Highlighted Files:</label
        >
              ...
              <input
                ...
                onchange={(e) => {
                  ...
                  if (checked) {
                    ...
                    currentStory.highlightedFiles = currentStory.highlightedFiles.filter(
                      (f) => f !== file
                    );
                  }
                }}
  ...
  <div class="border-t border-border pt-6 text-sm text-muted-foreground">
    ...
    <ul class="list-disc list-inside space-y-1">
      <li>
        <strong>Commit-by-commit view:</strong> Each commit is shown separately with its own metadata
        and file changes
      </li>
      <li>
        <strong>File status indicators:</strong> Added (A), Modified (M), Deleted (D), Renamed (R) files
        are clearly marked
      </li>
      <li>
        <strong>Split diff view:</strong> Side-by-side old/new line numbers with change indicators
      </li>
      ...
      <li>
        <strong>File highlighting:</strong> Specific files can be highlighted and auto-expanded
      </li>
      <li><strong>Caching:</strong> Loaded diffs are cached to avoid redundant requests</li>
      <li>
        <strong>Virtualization:</strong> Large commit lists can be virtualized for performance
      </li>
    ...
    <ul class="list-disc list-inside space-y-1">
      <li>
        <strong>Code review:</strong> Review changes commit-by-commit to understand the development narrative
      </li>
