...
export function mergeRepoStateByMaintainers(
  ctx: RepoContext,
...
export function getPatchGraph(ctx: RepoContext): {
...
export function getEffectiveLabelsFor(
  ctx: RepoContext,
  target: { id?: string; address?: string; euc?: string }
): EffectiveLabelsV2 {
  const selfEvt = target.id
  ...
  const legacyT = new Set<string>(
    (selfEvt?.tags || []).filter((t: any[]) => t[0] === "t").map((t) => t[1])
  );
  return mergeEffectiveLabelsV2({ self, external, t: Array.from(legacyT) }) as EffectiveLabelsV2;
}

export function getIssueLabels(ctx: RepoContext, rootId: string): EffectiveLabelsV2 {
  return getEffectiveLabelsFor(ctx, { id: rootId });
}
export function getPatchLabels(ctx: RepoContext, rootId: string): EffectiveLabelsV2 {
  return getEffectiveLabelsFor(ctx, { id: rootId });
}
...
export function getMaintainerBadge(
  ctx: RepoContext,
  pubkey: string
