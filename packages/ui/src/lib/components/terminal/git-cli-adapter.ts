...
export interface GitCliResult { code: number; stdout?: string; stderr?: string }
  ...
  switch (sub) {
    case 'status': {
      ...
      try {
        ...
        return { code: 0, stdout: String(res?.text ?? res ?? '') + (String(res?.text ?? res ?? '').endsWith('\n') ? '' : '\n') };
      } catch (e: any) {
    ...
    case 'log':
      ...
      try {
        ...
        const depth = depthIdx !== -1 && args[depthIdx+1] ? parseInt(args[depthIdx+1]!, 10) : 50;
        const branchArg = args.find(a => !a.startsWith('-'));
        const res = await ctx.rpc('git.log', { repoId: ctx.repoRef.repoId, branch: branchArg, depth, oneline });
    ...
    case 'push': {
      ...
      try {
        const res = await ctx.rpc('git.push', { repoId: ctx.repoRef.repoId, remote, branch, force });
        const text = typeof res === 'string' ? res : String(res?.text ?? '');
        return { code: 0, stdout: text ? (text.endsWith('\n') ? text : text + '\n') : '' };
      } catch (e: any) {
