...
export type UIToWorker =
  | { type: 'config'; repoRef: { relay: string; naddr: string; npub: string; repoId: string }; urlAllowlist?: string[]; outputLimit?: { bytes: number; lines: number; timeMs: number } }
...
function resolvePath(base: string, rel: string): string {
  if (!rel || rel === '/') return '/';
  return rel.startsWith('/') ? normalizePath(rel) : normalizePath((base.replace(/\/$/, '')) + '/' + rel);
}
...
function makeId(): string {
  ...
  if (g?.crypto && typeof g.crypto.randomUUID === 'function') {
    try { return g.crypto.randomUUID(); } catch (e) { /* randomUUID not available/use fallback */ }
  }
...
async function runBuiltin(id: string, argv: string[]): Promise<number> {
  ...
  switch (cmd) {
    ...
    case 'ls': {
      const targetArg = argv.find(a => !a.startsWith('-')) && argv.find(a => !a.startsWith('-')) !== 'ls'
    ...
    case 'cat': {
      const target = argv[1] ? (argv[1].startsWith('/') ? argv[1] : (cwd.replace(/\/$/, '') + '/' + argv[1])) : null;
      if (!target) { stderr(id, 'cat: missing file\n'); return 1; }
      try {
    ...
    case 'mkdir': {
      const target = argv[1] ? (argv[1].startsWith('/') ? argv[1] : (cwd.replace(/\/$/, '') + '/' + argv[1])) : null;
      if (!target) { stderr(id, 'mkdir: missing operand\n'); return 1; }
      try { await fsReq('mkdir', [target]); return 0; } catch (e: any) { stderr(id, `mkdir: ${e.message || e}\n`); return 1; }
    }
    case 'rm': {
      const recursive = argv.includes('-r') || argv.includes('-R');
      const files = argv.filter(a => !a.startsWith('-')).slice(1);
      if (!files.length) { stderr(id, 'rm: missing operand\n'); return 1; }
      for (const f of files) {
        const target = f.startsWith('/') ? f : (cwd.replace(/\/$/, '') + '/' + f);
        try { await fsReq('rm', [target, { recursive }]); } catch (e: any) { stderr(id, `rm: ${e.message || e}\n`); return 1; }
      }
      return 0;
    }
    case 'mv': {
      const [src, dst] = argv.slice(1);
      if (!src || !dst) { stderr(id, 'mv: missing file operand\n'); return 1; }
      const s = src.startsWith('/') ? src : (cwd.replace(/\/$/, '') + '/' + src);
      const d = dst.startsWith('/') ? dst : (cwd.replace(/\/$/, '') + '/' + dst);
      try { await fsReq('mv', [s, d]); return 0; } catch (e: any) { stderr(id, `mv: ${e.message || e}\n`); return 1; }
    }
    case 'cp': {
      const [src, dst] = argv.slice(1);
      if (!src || !dst) { stderr(id, 'cp: missing file operand\n'); return 1; }
      const s = src.startsWith('/') ? src : (cwd.replace(/\/$/, '') + '/' + src);
      const d = dst.startsWith('/') ? dst : (cwd.replace(/\/$/, '') + '/' + dst);
      try { await fsReq('cp', [s, d]); return 0; } catch (e: any) { stderr(id, `cp: ${e.message || e}\n`); return 1; }
    }
    case 'head':
    case 'tail': {
      const file = argv[1] ? (argv[1].startsWith('/') ? argv[1] : (cwd.replace(/\/$/, '') + '/' + argv[1])) : null;
      const nOptIdx = Math.max(argv.indexOf('-n'), argv.indexOf('--lines'));
      const n = nOptIdx !== -1 ? parseInt(argv[nOptIdx+1] || '10', 10) : 10;
      if (!file) { stderr(id, `${cmd}: missing file\n`); return 1; }
      try {
        ...
        if (!data.endsWith('\n')) stdout(id, '\n');
        return 0;
      } catch (e: any) { stderr(id, `${cmd}: ${e.message || e}\n`); return 1; }
    }
    case 'touch': {
      const file = argv[1] ? (argv[1].startsWith('/') ? argv[1] : (cwd.replace(/\/$/, '') + '/' + argv[1])) : null;
      if (!file) { stderr(id, 'touch: missing file\n'); return 1; }
      try { await fsReq('touch', [file]); return 0; } catch (e: any) { stderr(id, `touch: ${e.message || e}\n`); return 1; }
    }
  }
...
async function route(id: string, argv: string[]): Promise<number> {
  ...
  try {
    ...
    if (cmd === 'curl' || cmd === 'wget') {
      ...
      if (oIdx !== -1) outPath = argv[oIdx+1] ? (argv[oIdx+1].startsWith('/') ? argv[oIdx+1] : (cwd.replace(/\/$/, '') + '/' + argv[oIdx+1])) : null;
      if (!urlArg) { stderr(id, `${cmd}: missing URL\n`); return 2; }
      const u = new URL(urlArg);
      if (urlAllowlist.length === 0) {
        if (u.protocol !== 'https:') { stderr(id, `${cmd}: only https:// allowed\n`); return 2; }
      } else {
        const allowed = urlAllowlist.some(p => urlArg.startsWith(p));
        if (!allowed) { stderr(id, `${cmd}: URL not allowed\n`); return 2; }
      }
      const ctrl = new AbortController();
      const t = setTimeout(() => ctrl.abort(), Math.min(outputLimit.timeMs, 15000));
      try {
        const r = await fetch(urlArg, { signal: ctrl.signal });
        if (!r.ok) { stderr(id, `${cmd}: HTTP ${r.status}\n`); return 22; }
        ...
        while (reader) {
          ...
          if (loaded > max) { stderr(id, `${cmd}: download exceeds 20MB limit\n`); return 27; }
        ...
        if (e?.name === 'AbortError') { stderr(id, `${cmd}: request timed out\n`); return 28; }
