...
const timeAgo = $derived(() => formatDistanceToNow(new Date(startedAt), { addSuffix: true }));
  ...
  <div class="git-card border border-border rounded-lg bg-card p-4 hover:border-primary/50 transition-all">
    <div class="flex items-start gap-3">
      <div class="relative">
        <PlayCircle class="h-6 w-6 {isActive ? 'text-green-500' : 'text-muted-foreground'}" />
        {#if isActive}
          <span class="absolute -top-1 -right-1 h-3 w-3 bg-green-500 rounded-full animate-pulse border border-background" />
        {/if}
      </div>
      ...
      <div class="flex-1">
        <Link to={`/git/repo/${repoId}/live/${id}`}>
          <h3 class="text-lg font-medium hover:text-primary transition-colors">{title}</h3>
        </Link>
        ...
        <div class="mt-4 flex items-center justify-between">
          ...
          <Avatar class="h-8 w-8">
            <Avatar.Image src={host.avatar} alt={host.name} />
            <Avatar.Fallback>{host.name.slice(0, 2).toUpperCase()}</Avatar.Fallback>
          </Avatar>
        </div>
      </div>
    </div>
