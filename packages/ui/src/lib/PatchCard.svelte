<script lang="ts">
    import { GitPullRequest, MessageSquare, Check, X,
             BookmarkPlus, BookmarkCheck,
             ChevronDown, ChevronUp } from 'lucide-svelte';
      ...
      <div class="flex-1">
        <div class="flex items-center justify-between">
          ...
          <div class="flex items-center gap-2">
            ...
            <button type="button" aria-expanded={isExpanded} aria-controls="patch-description" class="p-0 m-0 bg-transparent border-0" onclick={() => isExpanded = !isExpanded}>
              {#if isExpanded}
        ...
        <div class="flex flex-wrap items-center gap-2 mt-1 text-xs text-muted-foreground">
          ...
          <span>{commitCount} commit{commitCount !== 1 ? 's' : ''} to
            <code class="bg-secondary/50 px-1 rounded ml-1">{baseBranch}</code>
          </span><span>•</span>
          <span>{timeAgo} by {author.name}</span>
        </div>
  
        <p id="patch-description" class="text-sm text-muted-foreground mt-3 {isExpanded ? '' : 'line-clamp-2'}">
          {description}
        </p>
