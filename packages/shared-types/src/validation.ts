...
export const RepoStateTagSchema = z.union([
  DTag,
  RefsTag,
  HeadRefTag
]);
...
export const AppliedAsCommitsTag = z.tuple([z.literal('applied-as-commits'), z.string()]).rest(z.string());
...
export const validateRepoAnnouncementTags = (tags: unknown) => RepoAnnouncementTagsSchema.safeParse(tags);
...
export const PatchEventSchema = NostrEventSchema.extend({
  ...
  if (!hasTagName(evt.tags as unknown[], 'a')) {
    ctx.addIssue({ code: z.ZodIssueCode.custom, message: "Patch must include an 'a' tag (address of repo announcement)" });
  }
});
...
export const IssueEventSchema = NostrEventSchema.extend({
  ...
  if (!hasTagName(evt.tags as unknown[], 'a')) {
    ctx.addIssue({ code: z.ZodIssueCode.custom, message: "Issue must include an 'a' tag (address of repo announcement)" });
  }
});
...
export const StatusEventSchema = NostrEventSchema.extend({
  ...
  if (!hasTagName(evt.tags as unknown[], 'e')) {
    ctx.addIssue({ code: z.ZodIssueCode.custom, message: "Status must include at least one 'e' tag (target event)" });
  }
  if (!hasTagName(evt.tags as unknown[], 'p')) {
    ctx.addIssue({ code: z.ZodIssueCode.custom, message: "Status should include at least one 'p' tag (participants)" });
  }
});

export const validateRepoAnnouncementEvent = (evt: unknown) => RepoAnnouncementEventSchema.safeParse(evt);
