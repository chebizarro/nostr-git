...
export function createCommentEvent(opts: CreateCommentOpts): CommentEvent {
  ...
  const { type: rootType, value: rootValue, kind: rootKind, pubkey: rootPubkey, relay: rootRelay } = opts.root;
  ...
  if (opts.parent) {
    const { type: parentType, value: parentValue, kind: parentKind, pubkey: parentPubkey, relay: parentRelay } = opts.parent;
