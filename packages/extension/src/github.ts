...
export function parseGitHubIssueURL(url: string = window.location.href): IssueInfo | null {
  try {
    ...
    try {
      ...
      if (Array.isArray(fileTree)) {
        const licenseFile = fileTree.find(
          (item: any) =>
            typeof item.name === "string" && item.name.toLowerCase().startsWith("license"),
        )
        ...
        const runtimeFile = fileTree.find((item: any) =>
          ["runtime.txt", ".python-version", ".node-version", ".nvmrc"].includes(item.name),
        )
        if (runtimeFile?.name) {
        ...
        const pkgJson = fileTree.find((item: any) => item.name === "package.json")
        if (pkgJson) {
          const rawLines = embedded?.payload?.blob?.rawLines
          if (Array.isArray(rawLines)) {
            const enginesLine = rawLines.find((line: string) => line.includes('"node"'))
            if (enginesLine) {
          ...
          const licenseLine = rawLines.find((line: string) => line.trim().startsWith('"license"'))
          if (licenseLine) {
...
export async function fetchRootCommitHash(owner: string, repo: string): Promise<string | null> {
...
export function parseSnippetLink(): SnippetData | null {
  try {
    ...
    const commitLink = document.querySelector("a.Link--secondary") as HTMLAnchorElement
