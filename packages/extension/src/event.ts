...
export async function fetchRepoEvent(relays: string[]): Promise<NostrEvent | undefined> {
  ...
  try {
    ...
    const candidates = (Array.isArray(events) ? events : []).filter(evt => {
      if (!shouldValidate()) return true
      const v = validateRepoAnnouncementEvent(evt as any)
      return v.success
    })
    const found = candidates.find(evt =>
      evt.tags.some(t => t[0] === "clone" && t[1].includes(`${owner}/${repo}`)),
    )
    return found
  } catch (err) {
 ...
 */
export async function createRepoAnnouncementEvent(relays: string[]): Promise<EventTemplate | null> {
...
export function createCodeSnippetEvent(
  ...
  const tags: string[][] = [
    ...
    ["description", snippetDesc ? snippetDesc.description : snipppetData.description],
    ["alt", `code snippet: ${snipppetData.filePath}`],
    ["repo", `https://${snipppetData.host}/${snipppetData.owner}/${snipppetData.repo}`],
  ]
...
export async function createCodeReferenceEvent(
  ...
  if (typeof permalink.startLine === "number" && typeof permalink.endLine === "number") {
    tags.push(["lines", permalink.startLine.toString(), permalink.endLine.toString()])
  } else if (typeof permalink.startLine === "number" && typeof permalink.endLine !== "number") {
...
export async function publishEvent(event: EventTemplate, relays: string[]): Promise<NostrEvent> {
  const pool = new SimplePool()
