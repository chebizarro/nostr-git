...
import {parseGitHubIssueURL, parsePermalink, parseSnippetLink} from "./github"
...
async function insertNostrIssuesCommand() {
  ...
  getRepoEvent().then(e => {
    if (e) {
      ...
      ;(button.querySelector("button") as HTMLButtonElement | null)?.setAttribute(
        "title",
        "Share Issue on Nostr",
      )
      // A11y
...
async function insertNostrRepoCommand() {
  ...
  getRepoEvent().then(e => {
    ...
    if (e) {
      ...
      ;(li.querySelector("button") as HTMLButtonElement | null)?.setAttribute(
        "title",
        "Open on gitworkshop.dev",
      )
      injectSvgInline(label, "svg/gitworkshop.svg", ["octicon", "mr-2"])
      injectSvgInline(smlLabel, "svg/gitworkshop.svg", ["octicon", "Button-visual"])
      // A11y
      ...
      const shareOnNostr = async () => {
        try {
          ...
          const okRepo = window.confirm("Publish Repository Announcement to Nostr?")
          if (!okRepo) {
          ...
          if (repoAnnouncement) {
            ...
            if (nevent) {
              ...
              const href = `${base}${nevent}`
              const label =
                nevent.length > 24 ? `${nevent.slice(0, 12)}…${nevent.slice(-8)}` : nevent
          ...
          showSnackbar("❌ Failed to publish repository events", "error")
        }
      }
      ...
      injectSvgInline(smlLabel, "svg/nostr-icon.svg", ["octicon", "Button-visual"])
      // A11y
...
async function injectNostrMenuCommand() {
  ...
  const rootItem =
    (copyPermalinkItem?.closest('[role="menuitem"]') as HTMLElement | null) ||
  ...
  if (rootItem) {
    ...
    if (firstVisibleItem && firstVisibleItem.closest('[role="menuitem"], li')) {
      firstVisibleItem
        .closest('[role="menuitem"], li')!
        .insertAdjacentElement("afterend", permalinkItem)
    } else {
      openMenu.appendChild(permalinkItem)
    }
  }
  ...
  if (permalinkItem.parentElement) {
    ...
    if (permalinkItem.closest('[role="menuitem"], li')) {
      ...
      firstVisibleItem
        .closest('[role="menuitem"], li')!
        .insertAdjacentElement("afterend", snippetItem)
    } else {
      openMenu.appendChild(snippetItem)
    }
  }
