...
import { resolveStatus, type LocalStatusEvent } from '../src/lib/status-resolver';

function makeStatus(id: string, kind: number, pubkey: string, created_at: number): LocalStatusEvent {
  const e = {
    id,
    kind,
    pubkey,
