...
import { NostrGitProvider } from '../src/nostr-git-provider';
...
describe('NostrGitProvider blossomMirror integration', () => {
  ...
  beforeEach(async () => {
    ...
    provider = new NostrGitProvider(mockGitProvider, mockNostrClient);
  ...
  it('performs Blossom mirror after successful push', async () => {
    ...
    mockGitProvider.push.mockResolvedValue({ success: true });

    // Mock BlossomFS pushToBlossom method
    const pushToBlossomSpy = vi.spyOn(fs, 'pushToBlossom').mockResolvedValue();
