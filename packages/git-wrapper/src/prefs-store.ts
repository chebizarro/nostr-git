...
export class MemoryProtocolPrefs implements ProtocolPrefsStore {
  ...
  set(id: string, url: string) { this.m.set(id, url); }
}
...
export class FileProtocolPrefs implements ProtocolPrefsStore {
  constructor(private fs: any, private filePath: string) {}
  ...
  get(id: string) { return this.readAll()[id]; }
...
export class LocalStorageProtocolPrefs implements ProtocolPrefsStore {
  constructor(private storage: Storage, private prefix = 'ngit:prefs:') {}
  get(id: string) { return this.storage.getItem(this.prefix + id) || undefined; }
  set(id: string, url: string) { this.storage.setItem(this.prefix + id, url); }
}
