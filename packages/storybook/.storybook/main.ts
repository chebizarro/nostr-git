...
export default {
  ...
  viteFinal: async (config: any, {configType}: any) => {
    ...
    const sbCoreCsfDist =
      sbCoreCsfPath || probe(["dist", "csf.js"]) || probe(["dist", "csf", "index.js"])
    ...
    const previewRuntimeResolved =
      sbPreviewRuntimeDist || sbPreviewRuntimePath || "@storybook/core/preview/runtime"
    ...
    config.plugins = [
      {
        ...
        resolveId(id: string) {
          ...
          if (bare === "@storybook/core/preview/runtime" && previewRuntimeResolved)
            return previewRuntimeResolved
        ...
        load(id: string) {
          if (id === "\u0000sb-csf-shim") {
            const coreCsfFile =
              sbCoreCsfDist && fs.existsSync(sbCoreCsfDist)
                ? sbCoreCsfDist
        ...
        configureServer(server: any) {
          server.middlewares.use((req: any, res: any, next: any) => {
            ...
            if (url.startsWith("/node_modules/storybook/")) {
              ...
              console.log(
                "[sb-core-id-redirect] generic @storybook wrapper for",
                req.url,
                "->",
                `@storybook/${rest}`,
              )
            ...
            if (url.startsWith("/node_modules/@storybook/")) {
              ...
              if (rest.endsWith(".cjs")) rest = rest.slice(0, -4)
              console.log(
                "[sb-core-id-redirect] scoped @storybook wrapper for",
                req.url,
                "->",
                `@storybook/${rest}`,
              )
            ...
            if (/^\/node_modules\/storybook\/core\/preview\/runtime(\.m)?js$/.test(url)) {
              console.log(
                "[sb-core-id-redirect] middleware serve preview runtime (via /@id) for",
                req.url,
              )
              res.setHeader("Content-Type", "application/javascript")
    ...
    config.resolve.conditions = Array.from(
      new Set([...(config.resolve.conditions || []), "svelte"]),
    )
    // Be friendly to pnpm symlinks when resolving files
    config.resolve.preserveSymlinks = true
    // Add .svelte to resolution list for import analysis
    config.resolve.extensions = Array.from(
      new Set([...((config.resolve && config.resolve.extensions) || []), ".svelte"]),
    )
